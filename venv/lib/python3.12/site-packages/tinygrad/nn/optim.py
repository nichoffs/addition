...
class Optimizer:
    def __init__(self, params: List[Tensor], lr: float):
        ...
        self.lr = (
            lr
            if getenv("CONST_LR")
            else Tensor([lr], requires_grad=False, device=self.device).contiguous()
        )
    ...
    def step(self):
        Tensor.corealize(self.schedule_step())
...
class OptimizerGroup(Optimizer):
    ...
    def _step(self) -> List[Tensor]:
        return [x for o in self.optimizers for x in o._step()]
...
class LAMB(Optimizer):
    def __init__(
        ...
        self.b1, self.b2, self.t = (
            Tensor([x], device=self.device, requires_grad=False).realize()
            for x in [b1, b2, 0]
        )
        self.m = [
            Tensor.zeros(
